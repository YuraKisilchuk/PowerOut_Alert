...
import { filterNonNull, ICityResult, ICitySearch } from "../types";

const countOnPage = 10;

const CitiesListPage = () => {
  const [searchParams, setSearchParams] = useSearchParams();
  const [search, setSearch] = useState<ICitySearch>({
  ...
  const [data, setData] = useState<ICityResult>({
    pages: 0,
    cities: [],
    total: 0,
    currentPage: 0,
  ...
  const deleteHandler = (id: number) => {
    http.delete("/api/cities/" + id).then((resp) => {
