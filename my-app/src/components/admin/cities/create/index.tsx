...
import { IAreaValue } from "../types";

const CreateCity = () => {
  const [alertOpen, setAlertOpen] = useState<boolean>(false);
  const [areas, setAreas] = useState<IAreaValue[]>([]);
  const navigator = useNavigate();

  const initValues: ICreateCity = {
    name: "",
    areaId: 1,
  };

  const onSubmitFormik = async (values: ICreateCity) => {
    try {
      const result = await http.post("/api/cities", values);
  ...
  useEffect(() => {
    http.get("/api/areas/selector").then((resp) => {
        setAreas(resp.data);
      });
  }, [])
